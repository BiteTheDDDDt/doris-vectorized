...
TEST_F(BloomFilterPredicateTest, bloom_filter_func_int_test) {
    ...
    std::unique_ptr<IBloomFilterFuncBase> func(
            create_predicate_function<BloomFilterTraits>(PrimitiveType::TYPE_INT, false,tracker.get()));
...
TEST_F(BloomFilterPredicateTest, bloom_filter_func_stringval_test) {
    auto tracker = MemTracker::CreateTracker();
    std::unique_ptr<IBloomFilterFuncBase> func(
            create_predicate_function<BloomFilterTraits>(PrimitiveType::TYPE_VARCHAR, false,tracker.get()));
    ASSERT_TRUE(func->init(1024, 0.05).ok());
    ObjectPool obj_pool;
    const int data_size = 1024;
    ...
    // test fixed char
    func.reset(create_predicate_function<BloomFilterTraits>(PrimitiveType::TYPE_CHAR, false,tracker.get()));
    ASSERT_TRUE(func->init(1024, 0.05).ok());
...
TEST_F(BloomFilterPredicateTest, bloom_filter_size_test) {
    auto tracker = MemTracker::CreateTracker();
    std::unique_ptr<IBloomFilterFuncBase> func(create_predicate_function<BloomFilterTraits>(PrimitiveType::TYPE_VARCHAR, false,tracker.get()));
    int length = 4096;
