    ...
    Status do_process_with_other_join_conjunts(HashTableContext& hash_table_ctx,
        ...
        for (int i = 0; i < column->size(); ++i) {
            ...
            Status process_data_in_hashtable(HashTableContext & hash_table_ctx,
                ...
                auto& mcol = mutable_block.mutable_columns();
                int right_col_idx =
                        _join_node->_is_right_semi_anti ? 0 : _left_table_data_types.size();
                int right_col_len = _right_table_data_types.size();
        ...
        HashJoinNode::HashJoinNode(ObjectPool * pool, const TPlanNode& tnode,
                ...
                : ExecNode(pool, tnode, descs),
                  ...
                  _match_all_build(_join_op == TJoinOp::RIGHT_OUTER_JOIN ||
                                   _join_op == TJoinOp::FULL_OUTER_JOIN),
                  _build_unique(_join_op == TJoinOp::LEFT_ANTI_JOIN ||
        ...
        Status HashJoinNode::init(const TPlanNode& tnode, RuntimeState* state) {
            ...
            _row_desc_for_other_join_conjunt =
                    RowDescriptor(child(0)->row_desc(), child(1)->row_desc());

            const bool build_stores_null = _join_op == TJoinOp::RIGHT_OUTER_JOIN ||
                                           _join_op == TJoinOp::FULL_OUTER_JOIN ||
                                           _join_op == TJoinOp::RIGHT_ANTI_JOIN;
            const bool probe_dispose_null = _match_all_probe || _build_unique ||
            ...
            if (tnode.hash_join_node.__isset.vother_join_conjunct) {
                ...
                RETURN_IF_ERROR(doris::vectorized::VExpr::create_expr_tree(
                        _pool, tnode.hash_join_node.vother_join_conjunct,
        ...
        Status HashJoinNode::get_next(RuntimeState * state, Block * output_block, bool* eos) {
            ...
            if ((probe_rows == 0 || _probe_index == probe_rows) && !_probe_eos) {
                _probe_index = 0;
                // TODO: support mem reuse and need remove when probe column is not nullale
                // and build column is nullable. The new created nullable column in probe block
                // need to be removed before call _probe_block.clear_column_data();
                _probe_block.clear();
            ...
            if (_probe_index < _probe_block.rows()) {
                        ...
                        [&](auto&& arg) {
                            ...
                            if constexpr (!std::is_same_v<HashTableCtxType, std::monostate>) {
                                if (_probe_ignore_null) {
                                    ProcessHashTableProbe<HashTableCtxType, true>
                                            process_hashtable_ctx(this, state->batch_size(),
                                                                  probe_rows);
                                    ...
                                    ProcessHashTableProbe<HashTableCtxType, false>
                                            process_hashtable_ctx(this, state->batch_size(),
                                                                  probe_rows);
                ...
                if (_is_right_semi_anti ||
                    (_is_outer_join && _join_op != TJoinOp::LEFT_OUTER_JOIN)) {
                            ...
                            [&](auto&& arg) {
                                ...
                                if constexpr (!std::is_same_v<HashTableCtxType, std::monostate>) {
                                    ...
                                    st = process_hashtable_ctx.process_data_in_hashtable(
                                            arg, mutable_block, output_block, eos);
                                } else {
                                    LOG(FATAL) << "FATAL: uninited hash table";
                                }
                            },
            ...
            for (size_t i = 0; i < _probe_expr_ctxs.size(); ++i) {
                ...
                if (_is_null_safe_eq_join[i]) {
                    ...
                    if (auto* nullable = check_and_get_column<ColumnNullable>(*column)) {
                        ...
                        if (_build_not_ignore_null[i]) {
                            auto column_ptr = make_nullable(
                                    block.get_by_position(result_col_id).column, false);
...
#define CALL_BUILD_FUNCTION(HAS_NULL, BUILD_UNIQUE)                                           \
    ...
    st = hash_table_build_process(arg, &null_map_val);
                            ...
                            } else if (std::pair {has_null, _build_unique} ==
                                       std::pair {true, false}) {
