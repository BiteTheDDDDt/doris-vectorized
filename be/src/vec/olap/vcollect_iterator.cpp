...
bool VCollectIterator::LevelIteratorComparator::operator()(LevelIterator* lhs, LevelIterator* rhs) {
    RETURN_NOT_OK(lhs->current_row(&lhs_ref));
    RETURN_NOT_OK(rhs->current_row(&rhs_ref));
...
OLAPStatus VCollectIterator::current_row(IteratorRowRef* ref) const {
    if (LIKELY(_inner_iter)) {
        return _inner_iter->current_row(ref);
    }
...
OLAPStatus VCollectIterator::Level0Iterator::current_row(IteratorRowRef* ref) const {
    ref->row_pos = _current_row;
    ref->block = &_block;
    ref->is_same = _is_same;
    return OLAP_SUCCESS;
}
...
OLAPStatus VCollectIterator::Level1Iterator::current_row(IteratorRowRef* ref) const {
    if (_cur_child != nullptr) {
        return _cur_child->current_row(ref);
    }
    return OLAP_ERR_DATA_EOF;
}
...
OLAPStatus VCollectIterator::Level1Iterator::_merge_next(IteratorRowRef* ref) {
    ...
    if (_skip_same && _cur_child->_is_same) {
