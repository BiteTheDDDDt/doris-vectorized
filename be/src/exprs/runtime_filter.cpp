...
public:
    ...
    Status init(const RuntimeFilterParams* params) {
        ...
        case RuntimeFilterType::MINMAX_FILTER: {
            _minmax_func.reset(create_predicate_function<MinmaxFunctionTraits>(_column_return_type,
                                                                               _vectorized_enable));
            break;
        }
        case RuntimeFilterType::BLOOM_FILTER: {
            _bloomfilter_func.reset(create_predicate_function<BloomFilterTraits>(_column_return_type, _vectorized_enable,_tracker));
    ...
    Status assign(const PBloomFilter* bloom_filter, const char* data) {
        ...
        _bloomfilter_func.reset(
            create_predicate_function<BloomFilterTraits>(PrimitiveType::TYPE_INT, false,_tracker));
    ...
    Status assign(const PMinMaxFilter* minmax_filter) {
        ...
        _minmax_func.reset(
                create_predicate_function<MinmaxFunctionTraits>(type, _vectorized_enable));
        switch (type) {
        case TYPE_BOOLEAN: {
