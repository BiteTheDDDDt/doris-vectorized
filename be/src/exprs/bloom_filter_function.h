...
using CurrentBloomFilterAdaptor = BlockBloomFilterAdaptor;
...
public:
    ...
    void insert(const StringRef& value) {
        if (type == TYPE_DATE || type == TYPE_DATETIME) {
            vectorized::DateTime date_time = *reinterpret_cast<const __int128*>(value.data);
            insert(reinterpret_cast<void*>(&date_time));
        }
        ...
        insert(reinterpret_cast<void*>(value.data));
    }
